...
public sealed partial class RoomsService
{
    ...
    public async FTask OnClientFrame(long userId, ulong frameNumber, List<Frame>? frames)
    {
        ...
        if (userId <= 0)
        {
            Log.Warning($"[Rooms] ClientFrame 忽略：userId 非法, userId={userId}, frame={frameNumber}");
            return;
        }
        ...
        {
            Log.Warning(
                $"[Rooms] ClientFrame 入窗失败: userId={userId}, roomId={room.RoomId}, frame={frameNumber}, ops={frames?.Count ?? 0}, state={room.State}, error={error}");
        }
    }
}
